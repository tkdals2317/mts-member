create table `mts-member-db`.member
(
...
create table `mts-member-db`.role
(
    ...
    constraint fk_role_member_001 foreign key (member_id) references `mts-member-db`.member (member_id)
